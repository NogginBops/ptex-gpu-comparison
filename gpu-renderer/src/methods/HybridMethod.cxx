	...
	void HybridMethod::init(int width, int height, GLenum mag_filter, GLenum min_filter, int max_anisotropy) {
		// setup color output buffer for reduced traverse method
		{
			color_attachment_desc color_desc = {
				"Attachment: hybrid.color (RGB32F_MS)",
				GL_RGB8,
				GL_RGB,
				GL_FLOAT,
				GL_REPEAT, GL_REPEAT,
			...
			depth_attachment_desc depth_desc = {
				"Attachment: hybrid.depth (DEPTH32F_MS)",
		...
		ptex_program = compile_shader("program: hybrid", "shaders/ptex.vert", "shaders/ptex_hybrid.frag");
		...
		clamp_sampler = create_sampler("sampler: hybrid.border", clamp_desc);
